<template>
  <div class="labelingPage">
    ...
    <div class="home-main-content" style="padding-bottom: 0; padding-top: 10px">
      <p style="font-size: 24px; margin-top: 10px">
        Patch Ghosting Artifact Labeling System
      </p>
        ...
        <div :class="this.imageWidth <= 1080 ? 'imagecontainer' : 'imagecontainer-column'
          ...
          <div class="imageName">
            <div class="images">
              <div v-for="i in patchRow" :key="i">
                <div v-for="j in patchColumn" :key="j">
                  <div class="labeled-border" :style="{
                    width: borderBoxResize + 'px',
                    height: borderBoxResize + 'px',
                    left: (j - 1) * borderBoxResize + 'px',
                    top: (i - 1) * borderBoxResize + 'px',
                  }" v-show="this.userLabeling[(i - 1) * patchColumn + (j - 1)] > 0">
                    {{ this.userLabeling[(i - 1) * patchColumn + (j - 1)] }}
                  </div>
                </div>
              </div>
              <img :src="serveOriginalImage()" ref="originalImg" :style="{
                width: resizeWidth + 'px',
                height: resizeHeight + 'px',
              }" class="imageStyle" />
              <div class="currentBorder" :style="{
                width: borderBoxResize + 'px',
                height: borderBoxResize + 'px',
                left: leftValue + 'px',
                top: topValue + 'px',
            ...
            <p style="font-size: 14px; margin-top: 10px">
              {{ this.imageOriginalNameList[this.imageIndex] }}
            </p>
          </div>
          <div class="imageName">
            <div class="images">
  ...
  ">
              ...
              <img :src="serveArtifactImage()" ref="artifactImg" :style="{
                width: resizeWidth + 'px',
                height: resizeHeight + 'px',
              }" class="imageStyle" />
              <div class="currentBorder" :style="{
        ...
        <div class="patchbtncontainer">
          <div>
            <div class="patch-container">
              <div class="patchName">
                <div class="selected-patch-image" :style="{
                  width: borderBox + 'px',
                  height: borderBox + 'px',
                }">
                  <img ref="originalImg" :src="serveOriginalImage()" class="selected-patch" :style="{
                    right: rightValue + 'px',
              ...
              <div class="patchName">
                <div class="selected-patch-image" :style="{
                  ...
                  <img ref="artifactImg" :src="serveArtifactImage()" class="selected-patch" :style="{
                    right: rightValue + 'px',
                    bottom: bottomValue + 'px',
                  }" alt="artifact" />
                </div>
              ...
              <div class="patchName">
                <div class="selected-patch-image" :style="{
                  ...
                  <img ref="differenceImg" :src="serveDifferenceImage()" class="selected-patch" :style="{
                    right: rightValue + 'px',
                    bottom: bottomValue + 'px',
                  }" alt="difference" />
                </div>
          ...
          <div class="btncontainer">
            <button v-for="a in 6" :key="a" @click="labeling(a - 1)" class="scoreButton"
              :class="{ pressed: this.isPressed === a - 1 }">
              {{ buttonString[a - 1] }}
            </button>
          </div>
          <div class="btncontainer">
            <button class="scoreButton" @click="changePreviousPage()">
              {{ buttonString[6] }}
            </button>
            <span style="margin: 0 10px">{{ imageIndex + 1 }} / {{ imageIndexList.length }}</span>
            <button class="scoreButton" @click="changeNextPage()">
              {{ buttonString[this.pageState] }}
            </button>
          </div>
        </div>
      </div>
...
export default {
  ...
  unmounted() {
    ...
    this.nextOriginalImage = null;
    this.nextArtifactImage = null;
    this.nextDifferenceImage = null;
    this.prevOriginalImage = null;
    this.prevArtifactImage = null;
    this.prevDifferenceImage = null;
  },
  ...
  methods: {
    ...
    preloadImage() {
      if (this.currentPage === 0) {
        ...
        this.nextOriginalImage.src = String(
          this.baseUrl + "postimage/original/" + (this.currentPage + 1)
        );
        this.nextArtifactImage.src = String(
          this.baseUrl + "postimage/artifact/" + (this.currentPage + 1)
        );
        this.nextDifferenceImage.src = String(
          this.baseUrl + "postimage/difference/" + (this.currentPage + 1)
        );
        ...
        this.prevOriginalImage.src = String(
          this.baseUrl + "postimage/original/" + (this.currentPage - 1)
        );
        this.prevArtifactImage.src = String(
          this.baseUrl + "postimage/artifact/" + (this.currentPage - 1)
        );
        this.prevDifferenceImage.src = String(
          this.baseUrl + "postimage/difference/" + (this.currentPage - 1)
        );
      } else {
        this.nextOriginalImage = new Image();
    ...
    async getImageIndexCurrentPage() {
      ...
      await axios
        ...
        .then((response) => {
          ...
          if (
            this.currentPage <= 0 ||
            this.currentPage > response.data.image_list.length - 1
          ) {
            this.currentPage = temp; //url로 접근하는데 범위 밖일 때
    ...
    addBorder(index) {
      ...
      if (this.userLabeling[index] > 0) {
        return {
          left: j * this.borderBox + "px",
          top: i * this.borderBox + "px",
        };
      }
      return {};
    },
    ...
    changePreviousPage() {
      ...
      if (this.imageIndex === 0) {
        ...
        this.$refs.originalImg = this.prevOriginalImage;
        this.$refs.artifactImg = this.prevArtifactImage;
        this.$refs.differenceImg = this.prevDifferenceImage;
        this.makeImageTemplete();
        this.getUserLabeling();
        this.preloadImage();
        this.$router.push({
    ...
    changeNextPage() {
      if (this.imageIndex >= this.imageIndexList.length - 1) {
        ...
        if (this.imageIndex === this.imageIndexList.length - 2)
          this.pageState = 8;
        this.postUserLabeling();
        this.imageIndex += 1;
        this.i = 0;
        this.j = 0;
        this.currentPage = this.imageIndexList[this.imageIndex];
        this.$refs.originalImg = this.nextOriginalImage;
        this.$refs.artifactImg = this.nextArtifactImage;
        this.$refs.differenceImg = this.nextDifferenceImage;
        this.makeImageTemplete();
        this.getUserLabeling();
        this.preloadImage();
        this.$router.push({
